...
rules: [/^(?:\s+)/,/^(?:^((H|h)(T|t)(T|t)(P|p)(S|s)?:\/\/))/,/^(?:^([M|m][A|a][I|i][L|l][T|t][O|o]:(\/\/)?(([0-9]|[a-z]|[A-Z])+((_|-|\.)*([0-9]|[a-z]|[A-Z])+)*)+@(([0-9]|[a-z]|[A-Z])+((-)*([0-9]|[a-z]|[A-Z])+)*)(\.(([0-9]|[a-z]|[A-Z])+((-)*([0-9]|[a-z]|[A-Z])+)*))*))/,/^(?:$)/,/^(?:(#[^ ]+))/,/^(?:(\/(([0-9]|[a-z]|[A-Z]|\+|_|-|'|\.|\(|\)|%20)+\/?)*))/,/^(?:([0-9]|[a-z]|[A-Z]|_)+=([0-9]|[a-z]|[A-Z]|_|\+|%20|-|\.)+)/,/^(?:-)/,/^(?:\.)/,/^(?::)/,/^(?:@)/,/^(?:\/)/,/^(?:\?)/,/^(?:&)/,/^(?:=)/,/^(?:_\b)/,/^(?:\+)/,/^(?:([0-9]|[a-z]|[A-Z])+)/,/^(?:(%20|%21|%23|%24|%26|%27|%28|%29|%2A|%2B|%2C|%2F|%3A|%3B|%3D|%3F|%40|%5B|%5D)+)/],
